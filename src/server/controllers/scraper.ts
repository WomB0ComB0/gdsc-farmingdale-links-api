...
export const scrapeEvents = async (): Promise<Events[]> => {
	...
	$("#--ruSKZrkro").each((_i, elem) => {
		const titleWithDate =
			$(elem).find("strong div.dynamic-text").text().trim() ||
			$(elem).find("div.dynamic-text div").text().trim();

		const title = titleWithDate.replace(/^.*?\b\d{1,2},?\s\d{4}/, "").trim();

		const thumbnailLink = $(elem).find("img").attr("src") || null;
		const detailsLink =
			$(elem).find("a.link-styles__link_1ec3q").attr("href") || null;
...
export const scrapePastEvents = async (): Promise<Events[]> => {
	...
	const events = await page.evaluate(() => {
		const eventElements = document.querySelectorAll(
			'[data-testid="container-block-24RneQKmFRW"]',
		);
		const eventsArray: Events[] = [];

		eventElements.forEach((elem) => {
			const titleElement = elem.querySelector(
				"a.link-styles__link_1ec3q .dynamic-text",
			);
			const title =
				titleElement?.textContent
					?.replace(/^\s*\w+\s+\d+,\s+\d+\s*-\s*/, "")
					.trim() || "";
